...
export function HunterClient() {
  ...
  async function confirm() {
    setReview(false)
    setStage("Registrando sua pesquisa…")
    ...
    try {
      ...
      setReview(false)
      setStage("Consultando as fontes e processando resultados…")
  ...
  return <div className={`${styles.surface} grid gap-6 xl:grid-cols-[1.05fr_.95fr]`}>
    ...
    <Card className="min-h-[560px] border-border/70"><CardHeader className="border-b"><div className="flex justify-between"><div><CardTitle>Resultados</CardTitle><p className="mt-1 text-sm text-muted-foreground">Evidências públicas ligadas à fonte.</p></div>{latest && <Badge variant="outline">{latest.results.length}</Badge>}</div></CardHeader><CardContent className="p-5">{busy ? <HunterActivity stage={stage} /> : !latest ? <div className="grid min-h-96 place-items-center text-center"><div><Radar className="mx-auto size-7 text-muted-foreground"/><h2 className="mt-4 font-semibold">Pronta para investigar</h2><p className="mt-2 text-sm text-muted-foreground">Configure e confirme a primeira busca.</p></div></div> : latest.status === "running" ? <HunterActivity stage="A pesquisa está em execução. Atualize para consultar o resultado." /> : latest.status === "failed" ? <p role="alert" className="py-16 text-center text-sm text-destructive">A pesquisa falhou. Revise as fontes e tente novamente.</p> : latest.status === "pending_confirmation" ? <p className="py-16 text-center text-sm text-muted-foreground">Pesquisa registrada, mas a execução não foi confirmada. Inicie uma nova pesquisa.</p> : <div className="space-y-3">{latest.results.map((result) => <a key={result.id} href={result.url} target="_blank" rel="noreferrer" className="block rounded-xl border p-4 hover:border-primary/50"><div className="flex gap-3"><span className="min-w-0 flex-1"><strong className="line-clamp-2 text-sm">{result.title}</strong><span className="mt-1 block text-xs text-primary">{sourceInfo[result.source].label}</span>{result.summary && <span className="mt-2 line-clamp-3 block text-xs leading-5 text-muted-foreground">{result.summary}</span>}</span><ExternalLink className="size-4 shrink-0"/></div></a>)}{!latest.results.length && <p className="py-16 text-center text-sm text-muted-foreground">Nenhum resultado público encontrado.</p>}</div>}</CardContent></Card>
