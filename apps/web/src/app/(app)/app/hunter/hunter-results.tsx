    ...
    <div className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className={styles.contactActions}>
          {result.source === "instagram" && <Button asChild variant="outline" className="h-10 gap-2"><a href={result.url} target="_blank" rel="noopener noreferrer"><Users className="size-4" />Abrir perfil<ExternalLink className="size-3" /></a></Button>}
...
export function HunterResults({ jobs, selected, busy, loading, stage, error, onRefresh, onClear, onSelect, onBroaden }: {
  ...
  return <Card className={styles.resultsCard}>
    ...
    <CardContent className="p-0">
      ...
      {busy ? <HunterActivity stage={stage} /> : loading && !selected ? <div role="status" className="grid min-h-80 place-items-center text-center text-sm text-muted-foreground"><div><Loader2 className="mx-auto mb-3 size-6 animate-spin motion-reduce:animate-none" />Carregando pesquisas salvas…</div></div> : !selected ? !error && <div className="flex min-h-72 flex-col items-center justify-center px-6 py-10 text-center"><Radar className="size-6 text-primary" /><h2 className="mt-4 text-lg font-semibold">Nenhuma pesquisa ainda</h2><p className="mt-1 text-sm text-muted-foreground">Defina o público ao lado para começar.</p></div> : <>
