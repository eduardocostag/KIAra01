...
function LeadRow({ result, location, historical }: { result: HunterResult; location: string | null; historical: boolean }) {
  ...
  return <article className="group min-w-0 px-4 py-5 transition-colors hover:bg-muted/30 sm:px-5" aria-label={result.title}>
    <div className="flex gap-3">
      <div className="hidden size-10 shrink-0 place-items-center rounded-lg border bg-muted/50 text-sm font-semibold text-muted-foreground sm:grid" aria-hidden="true">{result.title.trim().slice(0, 1).toUpperCase()}</div>
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <h3 className="min-w-0 flex-1 break-words text-sm font-semibold leading-6">{result.title}</h3>
        ...
        {(opportunity !== null || data?.match_reasons?.length) && <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border bg-muted/25 p-3">
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">{whatsapp ? "Link de WhatsApp identificado na fonte pública. Nenhuma mensagem enviada." : phone ? "Telefone público · disponibilidade no WhatsApp não confirmada." : "A Kiara não inventa contatos ausentes."}</p>
...
export function HunterResults({ jobs, selected, busy, loading, stage, error, onRefresh, onSelect, onBroaden }: {
  ...
  return <Card className="min-w-0 gap-0 py-0 shadow-none ring-1 ring-border">
    <CardHeader className="gap-4 border-b p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><p className="mb-1 text-[10px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Seu espaço de prospecção</p><CardTitle className="font-semibold">Resultados da pesquisa</CardTitle></div>
    ...
    <CardContent className="p-0">
      ...
      {busy ? <HunterActivity stage={stage} /> : loading && !selected ? <div role="status" className="grid min-h-80 place-items-center text-center text-sm text-muted-foreground"><div><Loader2 className="mx-auto mb-3 size-6 animate-spin motion-reduce:animate-none" />Carregando pesquisas salvas…</div></div> : !selected ? !error && <div className="flex min-h-96 flex-col items-center justify-center px-6 py-12 text-center"><div className="grid size-14 place-items-center rounded-xl border bg-muted/50"><Radar className="size-6 text-primary" /></div><h2 className="mt-5 text-lg font-semibold tracking-tight">Sua próxima oportunidade começa aqui</h2><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Defina um público e os contatos que precisa. Os resultados aparecerão aqui, com evidências e conexão ao seu pipeline.</p><div className="mt-8 flex flex-wrap items-center justify-center gap-2 text-xs text-muted-foreground"><span className="rounded-md bg-muted px-2 py-1">Pesquisar</span><ArrowRight className="size-3" /><span className="rounded-md bg-muted px-2 py-1">Verificar</span><ArrowRight className="size-3" /><span className="rounded-md bg-muted px-2 py-1">Organizar no CRM</span></div><p className="mt-6 text-xs text-muted-foreground">Nenhuma pesquisa registrada</p></div> : <>
        <div className="border-b px-4 py-5 sm:px-5">
          ...
          {selected.validation && <dl className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-xl border bg-border text-center sm:grid-cols-4"><div className="bg-card p-3"><dd className="font-mono text-xl font-medium tabular-nums">{selected.results.length}</dd><dt className="mt-1 text-[10px] text-muted-foreground">Leads validados</dt></div><div className="bg-card p-3"><dd className="font-mono text-xl font-medium tabular-nums">{withPhone}</dd><dt className="mt-1 text-[10px] text-muted-foreground">Com telefone</dt></div><div className="bg-card p-3"><dd className="font-mono text-xl font-medium tabular-nums">{noSite}</dd><dt className="mt-1 text-[10px] text-muted-foreground">Sem site listado</dt></div><div className="bg-card p-3"><dd className="font-mono text-xl font-medium tabular-nums">{highOpportunity}</dd><dt className="mt-1 text-[10px] text-muted-foreground">Oportunidade digital</dt></div></dl>}
        ...
        <div className="divide-y">{selected.results.map((result) => <LeadRow key={result.id} result={result} location={selected.location} historical={historical} />)}</div>
