...
export function HunterResults({ jobs, selected, busy, loading, stage, error, activeQuery, activeLocation, activeSources, onRefresh, onClear, onSelect, onBroaden }: {
  ...
  return <Card className={styles.resultsCard}>
    ...
    <CardContent className="p-0">
      ...
      {busy ? <HunterActivity stage={stage} query={selected?.query || activeQuery} location={selected?.location || activeLocation} sources={selected?.sources || activeSources} foundCount={selected?.results.length ?? 0} /> : loading && !selected ? <div role="status" className="grid min-h-80 place-items-center text-center text-sm text-muted-foreground"><div><Loader2 className="mx-auto mb-3 size-6 animate-spin motion-reduce:animate-none" />Carregando pesquisas salvas…</div></div> : !selected ? !error && <div className={styles.discoveryIntro}><div className={styles.discoveryScene} aria-hidden="true"><span className={styles.discoveryOrbit} /><span className={styles.discoveryOrbitAlt} /><span className={styles.discoveryOrb}><i /><i /></span><span className={`${styles.sourceBubble} ${styles.instagramBubble}`}><AtSign /></span><span className={`${styles.sourceBubble} ${styles.mapsBubble}`}><MapPin /></span><span className={`${styles.sourceBubble} ${styles.webBubble}`}><Globe2 /></span></div><h2>Defina o público e deixe que eu encontro as oportunidades para você.</h2><div className={styles.discoveryBenefits}><div><span><Globe2 /></span><strong>Múltiplas fontes</strong><small>Instagram, Maps, sites e mais</small></div><div><span><Filter /></span><strong>Filtros inteligentes</strong><small>Encontre exatamente o seu público</small></div><div><span><Radar /></span><strong>Resultados organizados</strong><small>Leads prontos para revisar</small></div></div></div> : <>
