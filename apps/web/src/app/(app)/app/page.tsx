...
import { ArrowRight, CheckCheck, Columns3, MessageCircle, Radar, Users } from "lucide-react"
...
export default async function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Indicadores da operação">{metrics.map(({ title, value, detail, icon: Icon }) => <Card key={title} className="shadow-none"><CardContent className="p-5"><div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">{title}</p><Icon className="size-4 text-muted-foreground" /></div><p className="mt-4 text-3xl font-semibold tracking-tight tabular-nums">{value}</p><p className="mt-2 text-xs text-muted-foreground">{detail}</p></CardContent></Card>)}</section>
