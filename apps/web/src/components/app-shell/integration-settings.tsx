...
import { ArrowRight, BarChart3, Bot, CheckCircle2, ExternalLink, KeyRound, Link2, Loader2, MessageCircle, SearchCheck, ShieldCheck, X } from "lucide-react"
...
const fields: Record<Provider, Field[]> = {
  ...
  hermes: [{ name: "instance_id", label: "ID exclusivo da instância", required: true, placeholder: "cliente-kiara-01" }, { name: "endpoint_url", label: "URL HTTPS da instância", required: true, placeholder: "https://hermes-cliente.example.com" }, { name: "api_key", label: "API_SERVER_KEY", secret: true, required: true }],
  mailerfind: [],
}
const info = {
  ...
  hermes: { name: "Hermes Agent", detail: "Instância autônoma isolada por cliente", href: "https://hermes-agent.nousresearch.com/docs/user-guide/features/api-server", icon: Bot },
...
export function IntegrationSettings({ adminMode = false, mailerFindResult }: { adminMode?: boolean; mailerFindResult?: MailerFindResult }) {
  ...
  async function verifyHermes() {
    setSaving("hermes"); setMessage(null)
    try {
      const response = await fetch("/api/integrations/hermes/capabilities", { cache: "no-store" })
      const body = await response.json()
      if (!response.ok || body?.platform !== "hermes-agent") throw new Error(body?.error?.message ?? "A instância Hermes não respondeu.")
      setMessage({ type: "ok", text: `Instância ${body.instance_id} respondeu. A conexão está pronta; nenhuma automação foi iniciada.` })
    } catch (error) { setMessage({ type: "error", text: error instanceof Error ? error.message : "Falha ao verificar Hermes." }) }
    finally { setSaving(null) }
  }
  return <section className="space-y-5">
    <div className={cn("grid gap-4", adminMode ? "md:grid-cols-2 xl:grid-cols-4" : "md:grid-cols-3")}>{providers.map((provider) => { const providerInfo = info[provider], Icon = providerInfo.icon, status = statuses.find((item) => item.provider === provider); return <button key={provider} type="button" onClick={() => { setSelected(provider); setMessage(null) }} className={cn("group relative overflow-hidden rounded-[28px] border bg-card p-6 text-left shadow-[var(--shadow-1)] transition duration-200 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[var(--shadow-2)]", selected === provider && "border-primary/30 ring-4 ring-primary/5")}><div className="absolute -right-10 -top-10 size-36 rounded-full bg-primary/8 blur-3xl" /><div className="relative flex items-start justify-between"><span className="grid size-12 place-items-center rounded-2xl bg-primary/10 text-primary"><Icon className="size-5" /></span><Badge variant={status ? "secondary" : "outline"} className="rounded-full">{provider === "mailerfind" && status?.is_global ? "Ativo para todos" : status ? "Configurado" : "Não conectado"}</Badge></div><h2 className="kiara-editorial relative mt-8 text-2xl">{providerInfo.name}</h2><p className="relative mt-2 text-sm text-muted-foreground">{providerInfo.detail}</p><span className="relative mt-7 flex items-center gap-2 text-xs font-semibold text-primary">{status ? "Revisar conexão" : "Configurar conexão"}<ArrowRight className="size-3.5 transition group-hover:translate-x-1" /></span></button> })}</div>
    {selected && <div className="overflow-hidden rounded-[28px] border bg-card shadow-[var(--shadow-2)]"><header className="flex items-center justify-between border-b p-6"><div><p className="text-xs font-semibold text-primary">Configuração protegida</p><h2 className="kiara-editorial mt-1 text-2xl">Conectar {info[selected].name}</h2></div><Button size="icon" variant="ghost" onClick={() => setSelected(null)} aria-label="Fechar configuração"><X /></Button></header><div className="p-6 sm:p-8">{message && <Alert variant={message.type === "error" ? "destructive" : "default"} className="mb-6"><ShieldCheck /><AlertTitle>{message.type === "ok" ? "Tudo certo" : "Revise os dados"}</AlertTitle><AlertDescription>{message.text}</AlertDescription></Alert>}{selected === "mailerfind" ? <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end"><div><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-primary/10 text-primary"><Link2 className="size-5" /></span><div><p className="font-semibold">Conexão global por OAuth</p><p className="text-sm text-muted-foreground">Endpoint MCP verificado: mcp.mailerfind.com</p></div></div><p className="mt-5 max-w-2xl text-sm leading-6 text-muted-foreground">Conecte uma única conta administrativa. Depois da autorização, o MailerFind ficará disponível para as pesquisas de todos os usuários da Kiara, sem compartilhar tokens ou exigir novos logins.</p><div className="mt-4 flex flex-wrap gap-2"><Badge variant="outline">Todos os usuários</Badge><Badge variant="outline">mailerfind:read</Badge><Badge variant="outline">mailerfind:write</Badge><Badge variant="outline">PKCE</Badge></div></div><div className="flex flex-col gap-2"><Button asChild size="lg"><a href="/api/admin/integrations/mailerfind/connect"><Link2 />{statuses.some((item) => item.provider === "mailerfind") ? "Reconectar para todos" : "Conectar para todos"}</a></Button><Button asChild variant="ghost" size="sm"><a href={info.mailerfind.href} target="_blank" rel="noreferrer">Ver documentação oficial<ExternalLink /></a></Button></div></div> : <form onSubmit={(event) => { event.preventDefault(); void save(selected, event.currentTarget) }} autoComplete="off"><div className="grid gap-5 md:grid-cols-2">{fields[selected].map((field) => <div className="grid gap-2" key={field.name}><Label htmlFor={`${selected}-${field.name}`}>{field.label}{field.required ? <span className="text-primary"> *</span> : <span className="font-normal text-muted-foreground"> · opcional</span>}</Label><div className="relative"><Input className="h-12 rounded-xl bg-background pr-10" id={`${selected}-${field.name}`} name={field.name} type={field.secret ? "password" : "text"} required={field.required} placeholder={statuses.find((item) => item.provider === selected)?.configured_fields.includes(field.name) ? "Já configurado — digite para substituir" : field.placeholder} autoComplete="new-password" />{field.secret && <KeyRound className="absolute right-3 top-4 size-4 text-muted-foreground" />}</div></div>)}</div><div className="mt-8 flex flex-col-reverse gap-3 border-t pt-6 sm:flex-row sm:justify-between"><Button asChild variant="outline"><a href={info[selected].href} target="_blank" rel="noreferrer">Gerar credenciais oficiais<ExternalLink /></a></Button><div className="flex flex-wrap gap-2">{selected === "hermes" && statuses.some((item) => item.provider === "hermes") && <Button type="button" variant="outline" disabled={saving !== null} onClick={() => void verifyHermes()}>Verificar instância</Button>}<Button type="submit" size="lg" disabled={saving !== null}>{saving ? <Loader2 className="animate-spin" /> : <CheckCircle2 />}Salvar conexão</Button></div></div></form>}{selected === "hermes" && <p className="mt-4 text-xs leading-5 text-muted-foreground">Uma instância exclusiva por cliente. A chave dá acesso ao agente e às ferramentas dele; configure permissões na própria instância. Salvar ou verificar não inicia tarefas.</p>}</div></div>}
  </section>
}
