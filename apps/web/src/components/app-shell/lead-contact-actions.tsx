...
function personalize(template: string, profile: SalesProfile | null, consumer: PipelineEntry["consumer"]): string {
  const values: Record<string, string> = { remetente: profile?.sender_name.trim() || "[seu nome ou empresa]", nome: consumer.display_name, nicho: consumer.research_query?.trim() || "seu segmento", cidade: consumer.address?.trim() || "sua região", oferta: profile?.offer.trim() || "uma solução para sua presença digital" }
...
export function LeadContactActions({ entry, onRecorded }: { entry: PipelineEntry; onRecorded?: () => void | Promise<void> }) {
  ...
  const message = personalize(template, profile, consumer)
  ...
  const ready = Boolean(profile?.sender_name.trim() && message.trim())

  async function loadProfile(open: boolean) {
    if (!open || profile || loading) return
    setLoading(true); setNotice("")
    try { const value = await salesRequest<SalesProfile>("/api/sales/profile"); setProfile(value); setTemplate(value.templates.first_contact || FALLBACK) }
  ...
  function openChannel(channel: "whatsapp" | "instagram") { setOpenedChannel(channel); setNotice("Canal aberto. Depois do envio, volte aqui e confirme para registrar no CRM."); void record("opened", channel).catch(() => undefined); if (channel === "instagram") void copyMessage() }
    ...
    <Dialog onOpenChange={(open) => void loadProfile(open)}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        {loading ? <div className="flex min-h-52 items-center justify-center"><LoaderCircle className="animate-spin" /><span className="ml-2">Carregando seus modelos…</span></div> : <div className="grid gap-5 py-2">
          {!profile?.sender_name ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-sm">Configure o nome do remetente em Configurações antes de abrir um canal.</div> : null}
        ...
        <DialogFooter className="flex-wrap"><Button variant="outline" onClick={() => void copyMessage()} disabled={!message.trim()}>{copied ? <Check /> : <Copy />}{copied ? "Copiada" : "Copiar"}</Button>{instagram ? <Button variant="outline" asChild><a href={ready ? instagram : undefined} target="_blank" rel="noreferrer" aria-disabled={!ready} onClick={(event) => { if (!ready) event.preventDefault(); else openChannel("instagram") }}><AtSign />Copiar e abrir Instagram</a></Button> : null}{whatsapp ? <Button asChild><a href={ready ? whatsapp.url : undefined} target="_blank" rel="noreferrer" aria-disabled={!ready} onClick={(event) => { if (!ready) event.preventDefault(); else openChannel("whatsapp") }}><MessageCircle />{whatsapp.confirmed ? "Abrir WhatsApp" : "Tentar no WhatsApp"}</a></Button> : null}{openedChannel ? <Button onClick={() => void confirmSent()} disabled={saving}>{saving ? <LoaderCircle className="animate-spin" /> : <Check />}Confirmar que enviei</Button> : null}</DialogFooter>
      </DialogContent>
    </Dialog>
