...
export function PipelineBoard({ initialEntries }: { initialEntries: PipelineEntry[] }) {
  ...
  const filtered = useMemo(() => { const needle = query.trim().toLocaleLowerCase("pt-BR"); return entries.filter((entry) => (stageFilter === "all" || entry.stage === stageFilter) && (!needle || `${leadDisplayName(entry.consumer)} ${entry.consumer.research_query ?? ""} ${entry.consumer.address ?? ""} ${entry.consumer.source ?? ""}`.toLocaleLowerCase("pt-BR").includes(needle))) }, [entries, query, stageFilter])
...
function PipelineRow({ entry, active, onSelect }: { entry: PipelineEntry; active: boolean; onSelect: () => void }) {
  ...
  return <li className="relative border-b last:border-b-0"><button type="button" onClick={onSelect} className={cn("grid w-full grid-cols-[36px_minmax(0,1fr)_110px_110px_40px] items-center gap-3 px-3 py-2.5 text-left transition hover:bg-primary/[.045]", active && "bg-primary/[.09] before:absolute before:inset-y-0 before:left-0 before:w-0.5 before:bg-primary")}><span className={cn("relative z-10 grid size-7 place-items-center rounded-full border bg-[#17152b] text-[9px] font-bold", theme.border, theme.text)}>{initials(name)}</span><span className="min-w-0"><strong className="block truncate text-xs font-semibold">{name}</strong><span className="mt-1 flex min-w-0 items-center gap-1.5"><span className="rounded border border-border/70 bg-background/50 px-1.5 py-0.5 text-[8px] font-bold text-muted-foreground">{sourceCode(entry.consumer.source)}</span><span className={cn("inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[8px] font-semibold", theme.border, theme.text)}><span className={cn("size-1 rounded-full", theme.dot)} />{stage.label}</span><span className="truncate text-[9px] text-muted-foreground">{entry.consumer.address || websiteLabel(entry.consumer.website_status)}</span></span></span><span className="hidden min-w-0 sm:block"><span className="block truncate text-[10px] font-medium">{entry.next_action || "Definir próximo passo"}</span><span className="mt-0.5 block text-[9px] text-muted-foreground">{formatDate(entry.next_action_at)}</span></span><time className="hidden text-[10px] text-muted-foreground sm:block" dateTime={entry.updated_at}>{formatDate(entry.updated_at)}</time><ArrowRight className={cn("ml-auto size-3.5 text-muted-foreground", active && "text-primary")} /></button></li>
}
