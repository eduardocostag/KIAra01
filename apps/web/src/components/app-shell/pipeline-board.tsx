...
export function PipelineBoard({ initialEntries }: { initialEntries: PipelineEntry[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    const matches = entries.filter((entry) =>
      (stageFilter === "all" || entry.stage === stageFilter) &&
      (!whatsappOnly || Boolean(entry.consumer.whatsapp_url)) &&
      (!needle || `${leadDisplayName(entry.consumer)} ${entry.consumer.research_query ?? ""} ${entry.consumer.address ?? ""} ${entry.consumer.source ?? ""}`.toLocaleLowerCase("pt-BR").includes(needle)))
  ...
  }, [entries, query, sortOrder, stageFilter, whatsappOnly])
  ...
  return <div className="mx-auto w-full max-w-[1680px] space-y-3">
    <section aria-label="Resumo do pipeline" className="-mx-4 flex snap-x gap-2 overflow-x-auto px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:mx-0 md:grid md:grid-cols-3 md:overflow-visible md:px-0 md:pb-0 xl:grid-cols-6">{pipelineStages.map((stage) => { const theme = stageTheme[stage.id]; return <button key={stage.id} type="button" onClick={() => setStageFilter((current) => current === stage.id ? "all" : stage.id)} className={cn("group min-w-[118px] snap-start rounded-xl border bg-card/80 px-3 py-2 text-left backdrop-blur transition hover:-translate-y-0.5 md:min-w-0 md:py-2.5 md:shadow-lg", theme.border, theme.glow, stageFilter === stage.id && "ring-1 ring-primary/70")}><span className="flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[.1em] text-muted-foreground md:gap-2 md:text-[10px] md:tracking-[.12em]"><span className={cn("size-1.5 rounded-full shadow-[0_0_8px_currentColor]", theme.dot)} />{stage.label}</span><span className="mt-1 flex items-end justify-between"><strong className="text-base leading-none tabular-nums md:text-xl">{counts[stage.id]}</strong><small className={cn("hidden text-[9px] font-semibold md:block", theme.text)}>{stage.id === "won" ? "concluídos" : "no fluxo"}</small></span></button> })}</section>
    <div className="flex flex-col gap-2 border-0 bg-transparent p-0 md:flex-row md:items-center md:rounded-xl md:border md:bg-card/70 md:p-2 md:backdrop-blur"><div className="flex flex-nowrap gap-1.5 md:flex-wrap"><button type="button" onClick={() => setStageFilter("all")} className={cn("shrink-0 rounded-lg border px-3 py-1.5 text-xs font-semibold", stageFilter === "all" ? "border-primary/60 bg-primary/15 text-primary" : "border-border bg-background/50 text-muted-foreground")}>Todos</button>{pipelineStages.slice(0, 5).map((stage) => <button key={stage.id} type="button" onClick={() => setStageFilter(stage.id)} className={cn("hidden shrink-0 rounded-lg border px-3 py-1.5 text-xs md:inline-flex", stageFilter === stage.id ? "border-primary/60 bg-primary/15 text-primary" : "border-border bg-background/50 text-muted-foreground")}>{stage.label}</button>)}<button type="button" aria-pressed={whatsappOnly} onClick={() => setWhatsappOnly((current) => !current)} className={cn("inline-flex shrink-0 items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition", whatsappOnly ? "border-emerald-400/50 bg-emerald-400/15 text-emerald-300" : "border-border bg-background/50 text-muted-foreground hover:border-emerald-400/35 hover:text-emerald-300")}><MessageCircle className="size-3.5" />Com WhatsApp</button></div><div className="flex min-w-0 gap-2 md:ml-auto md:flex-1 md:justify-end"><div className="relative min-w-0 flex-1 md:max-w-xs"><Search className="absolute left-3 top-2.5 size-3.5 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="h-9 rounded-lg bg-background/60 pl-8 text-xs md:h-8" placeholder="Buscar lead..." aria-label="Buscar leads no Pipeline" /></div><Button size="icon-sm" variant="outline" className="size-9 shrink-0 md:hidden" aria-label="Atualizar Pipeline" onClick={() => void refresh()} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : <RefreshCw />}</Button><Button size="sm" variant="outline" className="hidden h-8 md:inline-flex" onClick={() => void refresh()} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : <RefreshCw />}Atualizar</Button></div></div>
...
function EmptyPipeline() { return <div className="grid min-h-56 place-items-center rounded-xl border border-dashed bg-card/50 p-6 text-center sm:min-h-72 sm:p-8"><div><CircleDollarSign className="mx-auto size-7 text-primary" /><h2 className="mt-3 text-base font-semibold sm:text-lg">Seu Pipeline começa com uma pesquisa</h2><p className="mt-2 text-sm text-muted-foreground">Encontre leads no Hunter e organize cada oportunidade aqui.</p><Button asChild className="mt-4 sm:mt-5"><Link href="/app/hunter">Pesquisar leads<ArrowRight /></Link></Button></div></div> }
