...
import { House, Inbox, Menu, Radar, Settings2, ShieldCheck, UsersRound } from "lucide-react"
...
export function AppShell({ children, isSystemAdmin = false }: { children: React.ReactNode; isSystemAdmin?: boolean }) {
  ...
  return <div className="kiara-workspace min-h-screen bg-background text-foreground">
    ...
    <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
      <SheetTrigger asChild><Button variant="outline" size="icon" className="fixed left-4 top-4 z-40 size-11 rounded-xl bg-background/90 shadow-lg backdrop-blur-xl md:hidden" aria-label="Abrir menu"><Menu /></Button></SheetTrigger>
      <SheetContent side="left" className="flex w-72 flex-col border-sidebar-border bg-sidebar p-5 text-sidebar-foreground">
        ...
        <div className="mt-7"><Navigation isSystemAdmin={isSystemAdmin} onNavigate={() => setMenuOpen(false)} /></div>
        <div className="kiara-shell-sidebar-footer mt-auto px-2 pt-5"><span className="text-[10px] font-semibold tracking-[.18em] text-sidebar-foreground/45">KIARA</span></div>
      </SheetContent>
    </Sheet>
    <div className="min-w-0 md:pl-[202px]">
      <main id="conteudo" tabIndex={-1} className="relative mx-auto min-h-screen w-full min-w-0 max-w-[1540px] p-4 pt-20 outline-none sm:p-7 sm:pt-20 md:pt-7 lg:p-8">{children}</main>
    </div>
  </div>
}
