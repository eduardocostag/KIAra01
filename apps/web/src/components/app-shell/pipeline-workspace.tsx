...
export function PipelineWorkspace({ initialEntries, initialError = "" }: { initialEntries: PipelineEntry[]; initialError?: string }) {
  ...
  return <section className="overflow-hidden rounded-[28px] border bg-card shadow-[var(--shadow-2)]">
    ...
    <div className="grid min-h-[650px] lg:grid-cols-[280px_minmax(400px,1fr)] xl:grid-cols-[280px_minmax(440px,1fr)_300px]">
      ...
      {selected && <main className="kiara-copilot-stage flex flex-col items-center justify-center border-b px-6 py-10 text-center lg:border-b-0 xl:border-r"><KiaraOrb size="lg" active /><p className="mt-7 text-xs font-semibold uppercase tracking-[.16em] text-primary">O que fazemos agora?</p><h2 className="kiara-editorial mt-3 max-w-xl text-4xl leading-tight">{selected.next_action || `Vamos conhecer ${selected.consumer.display_name}?`}</h2><p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">Revise os sinais públicos, escolha o estágio e abra o WhatsApp somente quando estiver pronto.</p><div className="mt-7"><LeadContactActions entry={selected} onRecorded={refresh} /></div><div className="mt-8 flex max-w-xl flex-wrap justify-center gap-1.5">{pipelineStages.map((stage, index) => <button key={stage.id} disabled={saving.includes(selected.id)} onClick={() => void move(selected, stage.id)} className={cn("rounded-full px-3 py-2 text-[11px] font-medium transition", stage.id === selected.stage ? "bg-primary text-primary-foreground shadow-md" : index <= pipelineStages.findIndex((item) => item.id === selected.stage) ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground hover:text-foreground")}>{stage.label}</button>)}</div></main>}
      {selected && <aside className="hidden p-6 xl:block"><div className="flex items-center justify-between"><p className="text-xs font-semibold text-muted-foreground">Dossiê do lead</p><Button asChild variant="ghost" size="sm"><Link href={`/app/leads/${encodeURIComponent(selected.consumer.id)}`}>Abrir<ArrowRight /></Link></Button></div><h3 className="kiara-editorial mt-6 text-2xl leading-tight">{selected.consumer.display_name}</h3>{selected.consumer.address && <p className="mt-3 flex gap-2 text-xs leading-5 text-muted-foreground"><MapPin className="mt-0.5 size-3.5 shrink-0" />{selected.consumer.address}</p>}<dl className="mt-8 space-y-5"><Fact icon={<MessageCircle />} label="Contato" value={selected.consumer.phone || "Não encontrado"} /><Fact icon={<Globe2 />} label="Presença digital" value={websiteLabel(selected.consumer.website_status)} /><Fact icon={<Sparkles />} label="Origem" value={sourceLabels[selected.consumer.source ?? ""] ?? "CRM"} />{selected.next_action_at && <Fact icon={<Clock3 />} label="Próxima ação" value={new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(selected.next_action_at))} />}</dl></aside>}
    </div>
  </section>
}

function Fact({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex gap-3 [&_svg]:mt-0.5 [&_svg]:size-4 [&_svg]:text-primary"><span>{icon}</span><div><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 break-words text-sm font-medium">{value}</dd></div></div> }
