...
import { parsePipeline, requestPipeline, sourceLabels, type PipelineEntry, type PipelineStage, websiteLabel } from "@/lib/api/pipeline"
...
export function PipelineWorkspace({ initialEntries, initialError = "" }: { initialEntries: PipelineEntry[]; initialError?: string }) {
  ...
  const [error, setError] = useState(initialError), [loading, setLoading] = useState(false)
  const refreshing = useRef(false)
  const visible = useMemo(() => entries.filter((entry) => `${entry.consumer.display_name} ${entry.consumer.phone ?? ""} ${entry.consumer.research_query ?? ""}`.toLocaleLowerCase("pt-BR").includes(query.toLocaleLowerCase("pt-BR"))), [entries, query])
