...
export function PipelineWorkspace({ initialEntries, initialError = "" }: { initialEntries: PipelineEntry[]; initialError?: string }) {
  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-3xl border bg-card shadow-[var(--shadow-2)]">
      <header className="relative overflow-hidden border-b bg-sidebar px-5 py-5 text-sidebar-foreground sm:px-7">
        <div className="pointer-events-none absolute -right-16 -top-24 size-56 rounded-full bg-primary/25 blur-3xl" />
        <div className="relative flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><p className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.18em] text-sidebar-foreground/60"><Sparkles className="size-3.5" />Kiara Command Center</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">Uma decisão por vez.</h2><p className="mt-1 text-sm text-sidebar-foreground/65">Escolha um lead; a Kiara mostra somente o que ajuda na próxima ação.</p></div><div className="flex flex-wrap gap-2"><div className="relative min-w-52 flex-1 xl:w-72"><Search className="absolute left-3 top-3 size-4 text-sidebar-foreground/45" /><Input className="h-10 border-white/10 bg-white/8 pl-9 text-white placeholder:text-white/40" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nome, telefone ou pesquisa" /></div><Button variant="secondary" onClick={() => void refresh()} disabled={loading || saving.length > 0}>{loading ? <Loader2 className="animate-spin" /> : <RefreshCw />}Atualizar</Button><Button asChild><Link href="/app/hunter"><Plus />Novo alvo</Link></Button></div></div>
      </header>
      ...
      <div className="grid min-h-[620px] lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        {selected ? <main className="min-w-0 p-5 sm:p-7"><div className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-wider text-primary">Lead em foco</p><h3 className="mt-2 break-words text-2xl font-semibold tracking-tight sm:text-3xl">{selected.consumer.display_name}</h3><p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span>{sourceLabels[selected.consumer.source ?? ""] ?? "CRM"}</span>{selected.consumer.address ? <><span>·</span><MapPin className="size-3.5" /><span>{selected.consumer.address}</span></> : null}</p></div><Button asChild variant="outline"><Link href={`/app/leads/${encodeURIComponent(selected.consumer.id)}`}>Ver dossiê<ArrowRight /></Link></Button></div>
          ...
          <div className="grid gap-3 sm:grid-cols-3"><Signal icon={<MessageCircle />} label="Contato" value={selected.consumer.whatsapp_url ? "WhatsApp confirmado" : selected.consumer.phone ? "Telefone disponível" : selected.consumer.instagram_username ? "Instagram disponível" : "Sem canal publicado"} /><Signal icon={<Globe2 />} label="Presença digital" value={websiteLabel(selected.consumer.website_status)} /><Signal icon={<Target />} label="Origem" value={selected.consumer.research_query || "Entrada manual"} /></div>
