...
class InfoPanel(QWidget):
    def __init__(self, loader: Callable[[], object], empty_message: str) -> None:
        super().__init__()
        self._loader, self._empty_message = loader, empty_message
...
class AutomationPanel(QWidget):
    ...
    def __init__(self, core) -> None:
        ...
        self.engine = core.background.automations
        ...
        self.trigger.setAccessibleName("Tipo de gatilho")
        self.trigger.addItems([kind.value for kind in TriggerKind])
        self.action = QComboBox()
        self.action.setAccessibleName("Ação da automação")
        self.action.addItems(core.tools.names())
        self.parameters = QLineEdit("{}")
        self.parameters.setAccessibleName("Parâmetros JSON")
        self.templates = QComboBox(
            accessibleName="Modelos de automação", objectName="automationTemplates"
        )
        self.templates.addItems(["abrir_portal_diariamente", "avisar_erro_detectado"])
        self.preview = QTextBrowser(
            accessibleName="Prévia da automação", objectName="automationPreview"
        )
        self.preview.setMaximumHeight(120)
        self.history = QTextBrowser(
            accessibleName="Histórico de execuções", objectName="automationHistory"
        )
        ...
        for label, widget in (
            ...
            ("Valor", self.trigger_value),
            ("Ação", self.action),
            ("Parâmetros", self.parameters),
        ):
            form.addRow(label, widget)
        buttons = QHBoxLayout()
        self._selection_buttons: list[QPushButton] = []
        for label, slot in (
            ("Aplicar modelo", self.apply_template),
            ("Ver prévia", self.show_preview),
            ("Ensinar rascunho", self.teach_draft),
            ("Criar", self.create),
            ("Ativar/desativar", self.toggle_selected),
            ("Excluir", self.delete_selected),
        ):
            button = QPushButton(label)
            button.setObjectName("automationAction")
            button.clicked.connect(slot)
            buttons.addWidget(button)
            if label in {"Ativar/desativar", "Excluir"}:
        ...
        subtitle = QLabel(
            "Escolha um modelo ou monte uma automação com revisão antes de ativar.",
            objectName="automationSubtitle",
        )
        ...
        template_row.addWidget(QLabel("Modelos recomendados", objectName="automationSection"))
        template_row.addWidget(self.templates, 1)
        layout.addLayout(template_row)
        layout.addWidget(QLabel("Configuração rápida", objectName="automationSection"))
        layout.addLayout(form)
        layout.addWidget(QLabel("Pré-visualização", objectName="automationSection"))
        layout.addWidget(self.preview)
        layout.addLayout(buttons)
        layout.addWidget(QLabel("Histórico de execuções", objectName="automationSection"))
    ...
    def apply_template(self) -> None:
        ...
        self.trigger.setCurrentText(spec.trigger_kind.value)
        self.trigger_value.setText(str(spec.interval_seconds or spec.trigger_value))
        self.action.setCurrentText(spec.action)
        self.parameters.setText(json.dumps(spec.action_parameters, ensure_ascii=False))
        self.show_preview()

    def teach_draft(self) -> None:
        try:
            spec = AutomationTeacher().prepare(
                name=self.name.text().strip(),
                action=self.action.currentText(),
                parameters=json.loads(self.parameters.text()),
            )
            self.preview.setPlainText(
                json.dumps(self.engine.preview(spec), ensure_ascii=False, indent=2)
            )
    ...
    def _draft(self) -> AutomationSpec:
        ...
        if kind == TriggerKind.RECURRING:
            kwargs = {"interval_seconds": float(raw)}
        elif kind == TriggerKind.SCHEDULED:
            kwargs = {"next_run_at": raw}
        return AutomationSpec(
            name=self.name.text().strip(),
            trigger_kind=kind,
            action=self.action.currentText(),
            action_parameters=json.loads(self.parameters.text()),
            **kwargs,
        )
    ...
    def show_preview(self) -> AutomationSpec | None:
        try:
            spec = self._draft()
            if not spec.name or not spec.action:
                raise ValueError("Nome e ação são obrigatórios")
            self.preview.setPlainText(
                json.dumps(self.engine.preview(spec), ensure_ascii=False, indent=2)
            )
            return spec
        except (ValueError, TypeError, json.JSONDecodeError) as exc:
            self.preview.setPlainText(f"Prévia inválida: {exc}")
            return None

    def create(self) -> None:
        spec = self.show_preview()
        if spec and _confirm(
            self, "Criar automação", f"Criar esta automação?\n\n{self.preview.toPlainText()}"
        ):
            try:
                self.engine.add(spec)
                self.refresh()
            except (ValueError, TypeError, json.JSONDecodeError) as exc:
                QMessageBox.warning(self, "Automação inválida", str(exc))
    ...
    def toggle_selected(self) -> None:
        ...
        if spec and _confirm(
            self,
            "Alterar automação",
            f"{('Desativar' if spec.enabled else 'Ativar')} “{spec.name}”? ",
        ):
            self.engine.set_enabled(spec.id, not spec.enabled)
            self.refresh()

    def delete_selected(self) -> None:
        spec = self._selected()
        if spec and _confirm(self, "Excluir automação", f"Excluir permanentemente “{spec.name}”? "):
            self.engine.remove(spec.id)
            self.refresh()
    ...
    def refresh(self) -> None:
        ...
        for spec in self.engine.store.list():
            ...
            item = QListWidgetItem(
                f"{state}: {spec.name} — {spec.trigger_kind.value} → {spec.action}"
            )
            item.setData(Qt.ItemDataRole.UserRole, spec.id)
            self.items.addItem(item)
        self.history.setPlainText(
            json.dumps(self.engine.store.list_runs(limit=20), ensure_ascii=False, indent=2)
        )
...
class AuditPanel(QWidget):
    ...
    def __init__(self, core) -> None:
        super().__init__()
        self.audit = core.tools.audit
        self.timeline = QTextBrowser(accessibleName="Linha do tempo de auditoria")
        refresh = QPushButton("Atualizar")
        refresh.clicked.connect(self.refresh)
        export = QPushButton("Exportar auditoria redigida")
    ...
    def refresh(self) -> None:
        self.timeline.setPlainText(
            json.dumps(self.audit.read(limit=200), ensure_ascii=False, indent=2)
        )

    def export(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Exportar auditoria", "kiara-audit.jsonl", "JSON Lines (*.jsonl)"
        )
...
class MemoryPanel(QWidget):
    def __init__(self, core) -> None:
        super().__init__()
        self.memory = getattr(core.context, "_memory", None)
        ...
        forget = QPushButton("Esquecer memória selecionada")
        forget.clicked.connect(self.forget_memory)
        ingest = QPushButton("Ingerir documento")
        ingest.clicked.connect(self.ingest_document)
        remove = QPushButton("Remover documento selecionado")
        remove.clicked.connect(self.forget_document)
        self._memory_action, self._document_action = forget, remove
        memory_label = QLabel("Memórias locais")
        memory_label.setBuddy(self.memories)
        document_label = QLabel("Base de conhecimento")
        document_label.setBuddy(self.documents)
        layout = QVBoxLayout(self)
    ...
    def refresh(self) -> None:
        ...
        if self.knowledge:
            for document in self.knowledge.list_documents():
                item = QListWidgetItem(
                    f"#{document['id']} {Path(document['source']).name} ({document['chunks']} trechos)"
                )
    ...
    def forget_memory(self) -> None:
        ...
        if (
            self.memory
            and item
            and _confirm(self, "Esquecer memória", f"Excluir permanentemente?\n\n{item.text()}")
        ):
            self.memory.forget(int(item.data(Qt.ItemDataRole.UserRole)))
            self.refresh()
    ...
    def ingest_document(self) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Ingerir conhecimento", "", "Documentos (*.txt *.md *.pdf)"
        )
        if path and _confirm(self, "Ingerir documento", f"Adicionar à base local?\n\n{path}"):
            try:
                self.knowledge.ingest(path)
                self.refresh()
            except (OSError, ValueError, RuntimeError) as exc:
                QMessageBox.warning(self, "Falha na ingestão", str(exc))
    ...
    def forget_document(self) -> None:
        ...
        if (
            ...
            and item
            and _confirm(
                self, "Remover conhecimento", f"Excluir documento e seus trechos?\n\n{item.text()}"
            )
        ):
            self.knowledge.forget_document(int(item.data(Qt.ItemDataRole.UserRole)))
            self.refresh()


class SettingsPanel(InfoPanel):
    def __init__(self, loader: Callable[[], object], core=None) -> None:
        super().__init__(loader, "Configuração indisponível")
        self.core = core
        self.autostart = QCheckBox("Iniciar Kiara ao entrar no Windows")
        self.autostart.setChecked(autostart.is_enabled())
        self.autostart.toggled.connect(self._change_autostart)
        self.layout().insertWidget(1, self.autostart)
        if core is not None:
            self.autonomy = QComboBox(accessibleName="Nível de autonomia")
            self.autonomy.addItems([mode.value for mode in AutonomyMode])
            self.autonomy.setCurrentText(core.tools.permission_gate.mode.value)
            apply = QPushButton("Aplicar autonomia nesta sessão")
            apply.clicked.connect(self._change_autonomy)
            self.layout().insertWidget(2, self.autonomy)
            self.layout().insertWidget(3, apply)

    def _change_autostart(self, enabled: bool) -> None:
        if enabled and not _confirm(
            self,
            "Ativar inicialização automática",
            "Permitir que a Kiara inicie ao entrar no Windows?",
        ):
    ...
    def _change_autonomy(self) -> None:
        mode, current = (
            AutonomyMode(self.autonomy.currentText()),
            self.core.tools.permission_gate.mode,
        )
        if mode == current:
            return
        if not _confirm(
            self,
            "Alterar autonomia",
            f"Alterar autonomia de “{current.value}” para “{mode.value}” nesta sessão?",
        ):
            self.autonomy.setCurrentText(current.value)
            return
        self.core.tools.permission_gate.mode = mode
        self.core.settings.raw.setdefault("autonomy", {})["mode"] = mode.value
        self.refresh()
...
def automation_snapshot(core) -> list[dict[str, object]]:
    ...
    return [
        {
            "name": i.name,
            "trigger": i.trigger_kind.value,
            "action": i.action,
            "enabled": i.enabled,
            "next_run_at": i.next_run_at,
        }
        for i in background.automations.store.list()
    ]


def memory_snapshot(core) -> dict[str, object]:
    memory, knowledge = (
        getattr(core.context, "_memory", None),
        getattr(core.context, "_knowledge", None),
    )
    return {
        "memory_enabled": memory is not None,
        "memory_records": memory.count() if memory else 0,
        "knowledge_enabled": knowledge is not None,
        "knowledge_documents": knowledge.document_count() if knowledge else 0,
...
def agent_snapshot(core) -> list[dict[str, object]]:
    ...
    return [
        {"name": a.name, "description": a.description, "context": sorted(a.context_keys)}
        for a in agents
    ]
...
def permission_snapshot(core) -> dict[str, object]:
    ...
    return {
        "autonomy_mode": tools.permission_gate.mode.value,
        "kill_switch_active": bool(tools.kill_switch and tools.kill_switch.stopped),
        "tools": [
            {"name": name, "permission": tools.permission_level(name).value}
